...
for path, x, y in (
    (old_tsv, old_x, old_y),
    (current_tsv, new_x, new_y),
    (reference_tsv, ref_x, ref_y),
):
...
max_abs_ref_log_ratio = np.round(
    np.max(np.abs(ref_log_ratio)), 4
)  # Maximum absolute log-ratio value
...
with txt.open("a") as f:
    f.write(
        f"{timestamp}\tRMS1={rms1}\tRMS2={rms2}\tMAX_ABS_BG_LOG_RATIO={max_abs_bg_log_ratio}\tMAX_ABS_REF_LOG_RATIO={max_abs_ref_log_ratio}\n"
    )
...
axes[1].set_title(
    f"Background log-ratio | RMS1: {rms1} | Max abs: {max_abs_bg_log_ratio}"
)
...
axes[3].plot(
    new_x_roi, ref_log_ratio, color="green", linewidth=0.8, label="Background log-ratio"
)
axes[3].grid(True, color="lightgray")
axes[3].set_xlim(max(max(ref_x), max(new_x)), min(min(ref_x), min(new_x)))
axes[3].set_title(
    f"Background log-ratio | RMS2: {rms2} | Max abs: {max_abs_ref_log_ratio}"
)
